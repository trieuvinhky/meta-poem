...
export default function TextEditor(props) {
  ...
  window.addEventListener("beforeunload", async function (e) {
    ...
    await MetaPoem_backend.removeFromActive(documentId, myPrincipal);
  });
  ...
  const wrapperRef = useCallback((wrapper) => {
    ...
    q.setContents({
      ops: [{ insert: startuptext, attributes: { bold: true } }],
    });
    setQuill(q);
  }, []);
  ...
  class MyPeer {
    constructor(recipient, myPrincipal) {
      ...
      if (this.recipient.length > 0) {
        setOffered((prevOffered) => [...prevOffered, this.recipient]);
      }
      ...
      this.peer.on("signal", async (data) => {
        if (this.recipient.length > 0) {
          await MetaPoem_backend.updateCurrentPeers(
            this.myPrincipal,
            this.recipient,
            data.type,
            data.sdp
      ...
      this.peer.on("connect", () => {
        ...
        setOffered((prevOffered) =>
          prevOffered.filter((item) => item !== this.recipient)
        );
      });
      ...
      this.peer.on("close", () => {
        ...
        setConnected((prevConnected) =>
          prevConnected.filter((item) => item !== this.recipient)
        );
        setConnectedPeers((prevConnected) =>
          prevConnected.filter((peer) => peer.recipient !== this.recipient)
        );
      });
  ...
  useEffect(() => {
    async function initialRun() {
      var currentActive = await MetaPoem_backend.getActiveUsers(documentId);
      if (!currentActive.includes(myPrincipal)) {
        await MetaPoem_backend.addToCurrentUsers(documentId, myPrincipal);
      }
    }
      ...
      if (peersActive.length != 0) {
        for (let i = 0; i < peersActive.length; i++) {
          ...
          if (myPrincipal != peersActive[i] && foundMe == true) {
            if (
              connected.indexOf(peersActive[i]) === -1 &&
              offered.indexOf(peersActive[i]) === -1
            ) {
  ...
  useEffect(() => {
    async function connectionRequests() {
      ...
      var request = await MetaPoem_backend.getConnectionRequest(myPrincipal);
  ...
  useEffect(() => {
    ...
    async function saveDocInterval() {
      await MetaPoem_backend.updateDocContents(
        documentId,
  ...
  function clearOfferNA(recipient) {
    ...
    if (connected.indexOf(recipient) === -1) {
      ...
      //kill the peer first
      let recipientPeer = myPeers.filter(function (e) {
        return e.recipient === recipient;
      });
      recipientPeer[0].getPeer().destroy();
      myPeers = myPeers.filter(function (e) {
        return e.recipient !== recipient;
      });
    }
  }
  ...
  function destroyPeer(recipient) {
    ...
    if (createdPeer) {
      var killPeer = myPeers.filter(function (e) {
        return e.recipient === recipient;
      });
  ...
  useEffect(() => {
    const sendDoc = async () => {
      var head = await MetaPoem_backend.getFirst(documentId);
  ...
  useEffect(() => {
    if (quillLoaded) {
      const retrieveDocContent = async () => {
        var docCon = await MetaPoem_backend.getDocContents(documentId);
